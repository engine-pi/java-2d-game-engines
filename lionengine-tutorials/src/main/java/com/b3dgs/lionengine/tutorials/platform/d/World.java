...
{
    /** Ground height. */
    static final int GROUND = 54;
    private static final ColorRgba BACKGROUND_COLOR = new ColorRgba(150, 190, 200);
    private static final Media HERO = Medias.create("entity", "Hero.xml");
    private static final Media MONSTER = Medias.create("entity", "Monster.xml");
    ...
    protected void loading(FileReading file) throws IOException
    {
        ...
        final Featurable hero = factory.create(HERO);
        handler.add(hero);
        tracker.track(hero);

        for (int i = 0; i < 3; i++)
        {
            final Featurable monster = spawn(MONSTER, 700 + i * 50, GROUND * 2);
            handler.add(monster);
        }
    }
}
